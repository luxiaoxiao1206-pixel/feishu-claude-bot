...
async function handleMessage(event) {
  try {
    ...
    // 方式1：明确提到图片
    const mentionsImageExplicit = /图片|这张|照片|截图|画面|图像/i.test(userMessage);

    // 方式2：使用分析类动词，且消息很短（可能是指代刚发的图片）
    // 如：@机器人 解读、@机器人 分析、@机器人 看看、@机器人 说说这是什么
    const analysisVerb = /^(解读|分析|看看|看|识别|描述|说说|讲讲|介绍|解释|读|认|这是啥|这是什么|什么内容).{0,15}$/i.test(userMessage);

    const mentionsImage = mentionsImageExplicit || analysisVerb;
...
重要：严格按照上述格式返回，标题和内容之间用 ===TITLE=== 和 ===CONTENT=== 分隔。`,
      ...
      try {
        ...
        if (!conversations || conversations.length === 0) {
          ...
          // 计算实际数据的时间范围
          const actualStartDate = stats.firstMessageTime ? new Date(stats.firstMessageTime).toLocaleDateString('zh-CN') : '未知';
          const actualEndDate = stats.lastMessageTime ? new Date(stats.lastMessageTime).toLocaleDateString('zh-CN') : '未知';

          // 使用 Claude AI 生成总结 - 改进的 prompt
          const summaryPrompt = `你是一个专业的工作报告分析助手。请深度分析以下${timeRange.period}的群聊记录（共${stats.userMessages}条用户消息）。
...
**第一步：数据提取与分类**
1. **识别工作报告**：提取所有包含工作内容、进展、任务的消息（忽略问候、祝福、闲聊等无关信息）
2. **按成员分类**：统计每个成员的发言次数和主要工作内容
3. **按日期分类**：梳理关键日期的重要事项
...
**注意**：请专注于工作相关内容，自动忽略问候、祝福、表情、闲聊等无关信息。`;

          const claudeResponse = await anthropic.messages.create({
            model: 'claude-opus-4-1-20250805',
            max_tokens: 8192,
            temperature: 0.7, // 提高创造性和洞察力（之前0.3太低，导致分析死板）
            system: `你是一位资深的数据分析专家和战略顾问，拥有10年以上的工作报告分析经验。

你的分析特点：
1. **深度洞察**：不只是总结表面信息，而是挖掘深层含义和趋势
2. **数据驱动**：从大量信息中提取关键数据和模式
3. **战略思维**：能够识别问题、机会和风险
4. **清晰表达**：用结构化、专业的方式呈现分析结果
...
请以专家的视角进行深度分析，而不是简单罗列信息。`,
          ...
          // 组合最终回复
          reply = `📅 ${timeRange.period} 工作总结报告\n\n` +
                  `📊 查询范围: ${new Date(timeRange.startTime).toLocaleDateString('zh-CN')} - ${new Date(timeRange.endTime).toLocaleDateString('zh-CN')}\n` +
                  `📅 实际数据: ${actualStartDate} - ${actualEndDate}\n` +
                  `💬 消息统计: ${stats.totalMessages} 条消息（用户 ${stats.userMessages} 条）\n\n` +
                  `━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
                  `${summaryText}\n\n` +
                  `━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
...
用户当前的问题：${userMessage}`;
        ...
        const claudeResponse = await anthropic.messages.create({
          model: 'claude-opus-4-1-20250805',
          max_tokens: 8192, // 提升回答长度上限
          temperature: 0.5, // 平衡创造性和准确性
          system: `你是一个运行在飞书平台上的高级 AI 助手，基于 Claude Opus 4.1 模型（Anthropic 最强大的模型）。
...
- 分析和处理飞书文档、多维表格、Wiki
- 创建和编辑飞书文档和表格
- 生成专业的工作报告（日报、周报、月报、年报）
- 汇总群聊文件和讨论内容
- 记住完整的对话历史（持久化存储）

## 时间范围总结（新功能）
- 月度总结：总结11月、总结2025年11月
- 周度总结：总结本周、上周
- 年度总结：总结2025年、今年
- 自定义范围：总结11月1日到15日、最近7天
